...
case class ClanSummary(clanId: Int, tag: String, emblem: String, membersCount: Int, skirmishBattles: Int, skirmishBattlesWins: Int, clanDelta: Option[ClanDelta] = None) {
  def totalWinRatio = BigDecimal((skirmishBattlesWins.toDouble / (if(skirmishBattles == 0) 1 else skirmishBattles).toDouble) * 100).setScale(2, BigDecimal.RoundingMode.HALF_DOWN).toDouble
}
